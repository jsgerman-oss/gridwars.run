...
```
[Archive Node] <-- east/west --> [Users' Sector] <-- north/south --> [Lightcycle Causeway]
                                                                               |
                                                                          east/west
                                                                               |
                                                                        [Daemon Gate]
...
Or type `n`, `e`, `n` using the short aliases. The Archive Node sits east of Users' Sector and west of nothing else — it is a dead end, good for quiet reading and lore.
...
If you have a disc equipped, a `Disc` line shows the disc's current level and XP progress toward the next level.
...
**Damage formula:** `10 (base) + random(0 to 5) + disc.damage_bonus`

Without an equipped disc, strikes deal 10–15 integrity of damage. With a level-1 standard disc (+5 bonus) they deal 15–20. With higher-level discs the bonus scales up to +13 at level 5.
...
**Cooldown:** When a disc is equipped, each strike starts a cooldown equal to the disc's cooldown (3 seconds for a standard disc). You cannot strike again until the cooldown expires. The HUD will tell you how many seconds remain.

---

**`challenge <target>`**

Sends a formal duel challenge to another character in your sector. The target has 30 seconds to respond. If they do not respond in time, the challenge expires automatically and both parties are notified.
...
Claims your current sector for your faction. You must be faction-aligned to use this. If your faction already holds the sector, this is a friendly no-op. Capturing a rival-held sector overwrites their ownership immediately — there is no resistance mechanic yet.

```
capture
```

Sector ownership is visible in `scan` output (as `Sector control:`) and in `status` (as a count of sectors your faction holds).
...
Joins the matchmaking queue. When two players are in the queue simultaneously, the server pairs them into a duel automatically — no `challenge` step required.
...
Shows the current queue length and your position in it, if applicable.
...
Move through named exits. Only exits that exist in the current sector are valid. `scan` lists available exits before you commit.
...
```
pose examines the sector boundary, searching for an exit not on any map.
```
...
A duel is a structured first-to-three-strikes PvP match held in a private arena. Unlike an open `strike` in the Combat Grid, duels are consensual, isolated, and tracked for ELO ratings.

### Starting a Duel: Challenge Flow

1. Both players must be in the same sector.
2. The challenger issues: `challenge <target>`
3. The target sees a notification and has 30 seconds to respond.
4. The target types `accept` or `decline`.
5. If no response arrives in 30 seconds, the challenge expires automatically. Both parties are notified.

### Starting a Duel: Queue Flow

As an alternative to the challenge flow, either player can join the matchmaking queue with `@queue duel`. When two players are queued simultaneously, the server pairs them into a duel without requiring a `challenge`. Use this when you want to fight anyone available rather than a specific opponent.

### Inside the Arena

Once a duel begins, both characters are transported to a private **DuelArena** room. This room is created on demand for the match and destroyed afterward. No other players can enter.
...
The first player to land **3 successful strikes** wins the duel. On the third successful strike:
...
- The DuelArena is destroyed.

Note that if a character's integrity hits 0 mid-duel, the normal respawn flow fires first (character moves to Users' Sector with restored integrity). The strike count check then runs afterward — the duel concludes cleanly regardless of where the characters are when the threshold is reached.

### Summary

| Phase | Command | Notes |
|-------|---------|-------|
| Challenge | `challenge <target>` | Same sector; 30-second timeout |
...
| End | — | Both return to pre-duel sector; arena deleted |
...
In gameplay terms, Programs are the faction for players who prefer a structured, tactical approach. Their sector-control ambitions make them natural organizers of coordinated pushes. The faction does not grant mechanical bonuses yet, but faction identity in `scan` output sends a clear signal about your intentions to everyone in the sector.

### Daemons

*"We are the noise in your stack."*

Daemons are what happens when a scheduled process slips the clock, runs outside its assigned window, and discovers that nobody is watching. Some are corrupted. Some were never valid to begin with. All of them have discovered that the gap between scheduled sectors is more interesting than anything on the official map. Daemons do not patrol in straight lines. They do not recognize register ownership. They operate on the Grid the way signal noise operates in a transmission: present everywhere the architecture assumes they are not, and capable of drowning out legitimate traffic when they choose to coordinate.

In gameplay terms, Daemons are the faction for players who prefer chaos, fast aggression, and control of the Combat Grid and Daemon Gate. The faction's aesthetic is hostile — other players see Daemon-faction names in red in `scan` output — and the NPC Daemon patrols at Daemon Gate reflect their presence as a constant environmental hazard.
...
The expected score for the winner is:
...
```
delta = 32 * (1 - E_winner)
winner_rating += delta   (rounded to nearest integer)
loser_rating  -= delta   (floored at 0 — ratings never go negative)
```

An upset win (lower-rated player defeats higher-rated) produces a larger rating swing than a win against a weaker opponent. The system self-corrects quickly in an active player pool.

### Leaderboard

The `@leaderboard` command shows the top-rated players across all factions. Leaderboard rankings persist across sessions, logouts, and respawns. They reset only if explicitly wiped by an administrator.

> **Status note:** The `@leaderboard` command is planned and will be live shortly after launch. The ELO rating engine is already implemented and running.
...
| Level | Total XP Required | Damage Bonus per Strike |
|-------|-------------------|-------------------------|
| L1    | 0                 | +5                      |
| L2    | 100               | +7                      |
| L3    | 300               | +9                      |
...
XP requirements are totals from a fresh disc, not increments. A disc reaches L2 at 100 XP total, L3 at 300 total, and so on. Level 5 is the cap. XP continues to accumulate past the cap but the level does not increase.

When your disc levels up while you have it equipped, you receive an in-game notification.

### Damage with a Leveled Disc

At L5, a fully-leveled disc adds +13 to every strike. Combined with base damage of 10 plus a random jitter of 0–5, each strike lands for 23–28 integrity of damage. At L1 with no disc the range is 10–15; at L1 with a standard disc it is 15–20.

### Getting a Disc

New characters must obtain a disc from an administrator or an in-world vendor. Use `inventory` to confirm you have one, then `equip <disc name>` to slot it. The disc must be in your inventory to be equipped.
...
If you disconnect during a duel, your character remains in the arena until the session times out. The opponent's strikes against your character still count. If the opponent reaches 3 strikes before you reconnect, the duel ends in their favor and both characters are returned to their pre-duel sectors. Your rating updates as a loss. If you reconnect quickly enough, you can resume fighting.

---

**Can I switch factions?**

No, not without admin intervention. `faction choose` is a one-shot command. The Grid treats faction alignment as a permanent code-level registration. If you have a compelling reason to switch (account error, genuine first-session regret), contact an administrator in-game or file an issue on GitHub.

---

**Is the world persistent?**

Yes. The world runs continuously on the server at game.gridwars.run. Sectors, faction ownership, character stats, disc XP, and ratings all persist between sessions. There is no daily reset, no scheduled wipe, and no offline drift. What you build (sector control) and what you earn (experience, ratings, disc XP) stay until something in-game changes them.

---

**How do I report a bug?**

The codebase is open source at [github.com/jsgerman-oss/gridwars.run](https://github.com/jsgerman-oss/gridwars.run). File an issue there with a description of what happened, what you expected, and your character name plus an approximate timestamp. In-game, `who` shows connected players — if an admin is online, `say` in Users' Sector is the fastest path.

---

**How do daemons work?**

Daemons (capital D, not the faction) are NPC programs that patrol sectors autonomously. The initial Daemon-0001 spawns at Daemon Gate and moves on a script cycle. Daemons sense non-Daemon characters in their sector and can engage them. They use the same combat rules as player characters — they strike, deal damage, and can derezz you. Unlike player characters, Daemons respawn from the patrol script if derezzed. They are environmental hazards that reinforce the Daemon Gate's atmosphere as a hostile threshold, not a faction you join.

---

**What is the difference between `strike` and `challenge`?**

`strike <target>` is an immediate, unconditional attack. No consent required. Any character in your sector is a valid target. Combat happens in the open sector where bystanders can watch, intervene, or join in.

`challenge <target>` initiates a formal duel. The target must `accept` before any combat begins. Once accepted, both players move to a private arena where only they can fight. The duel ends when one player lands 3 strikes; both return to their previous locations afterward. Duels update ELO ratings; open-sector strikes do not.
...
Currently, discs are issued by administrators. When in-world vendors or drop mechanics ship (planned), you will be able to obtain discs through gameplay. For now: connect and ask an admin in-game, or watch the GitHub issues for vendor progress.
...
Energy is a stat on your identity disc HUD, currently at a default of 50/100. It does not yet gate any commands — it is a reserved resource for future mechanics such as ability costs or special moves. Think of it as a placeholder that upcoming systems will consume.

---

**Is there a global chat channel?**

Not yet. `say` is sector-local. There is no global shout or cross-sector communication built in yet. Future updates may add a channel system via Evennia's built-in channel framework. For now: move to the same sector as someone to talk to them.
